...
export async function GET(request: NextRequest){
  try{
    ...
    if(!paginationResult.success){
      throw createApiError(400, ErrorCodes.VALIDATION_ERROR, 'Paramètres de pagination invalides')
    }
    ...
    if(!searchResult.success){
      throw createApiError(400, ErrorCodes.VALIDATION_ERROR, 'Paramètres de recherche invalides')
    }
